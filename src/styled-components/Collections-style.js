...
CollectionStyle.changeColor = () => {
    var colors = [`#21CE99`, `#F45531`];
    return colors[Math.round(Math.random() * (1 - 0) + 0)]
}

CollectionStyle.color = CollectionStyle.changeColor();

CollectionStyle.changeContainerColor = () => {
    if (CollectionStyle.color === `#21CE99`) {
...
CollectionStyle.containerColor = CollectionStyle.changeContainerColor();
...
display:inline-block;
border-radius: 25px;
background: ${CollectionStyle.containerColor};
padding: 8px; 
text-align: center;
height: 15px;
width: fit-content;
...
font-size: 13px;
:hover {
    background: ${CollectionStyle.color};
    color: black
}
`;
